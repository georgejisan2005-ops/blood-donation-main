...
const donorAuth = async (req, res, next) => {
  try {
    await auth(req, res, () => {
      if (!req.user.roles || (!req.user.roles.includes('donor') && !req.user.roles.includes('admin'))) {
...
const recipientAuth = async (req, res, next) => {
  try {
    await auth(req, res, () => {
      if (!req.user.roles || (!req.user.roles.includes('recipient') && !req.user.roles.includes('admin'))) {
